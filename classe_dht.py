...
class DHT:
	...
	def __init__(self):
		...
		serv.bind((host, porta)) # Espera conecoes no endereco e porta fornecidos
		print "Servidor rendezvous iniciado..."
		msg = ""
		while msg != "QSAIR": 
			msg, ender = serv.recvfrom(1024)
			...
			if (msg == 'Hello') and (len(self.nos) < MAX_NO - 1):
				...
				if not self.root: # se o dicionario esta vazio
					...
					print "Root serv"
					env = 'root' +'|'+ str(id) +'|'+ str(ender[0]) +'|'+ str(ender[1])
					serv.sendto(env, ender) # informa ao no que ele eh o root
				else:
					rootID = self.root.keys()
					rootIP, rootPorta = self.root[rootID[0]]
					
					env = 'novo' +'|'+ str(rootID[0]) +'|'+ str(rootIP) +'|'+ str(rootPorta) +'|'+ str(id)
					serv.sendto(env, ender) # informa ao no qual no eh o root
					
			elif len(self.nos) == MAX_NO - 1:
				print "A rede ja esta cheia!\n"
				serv.sendto("cheio", ender) # informa a um no que deseja entrar na rede  que a rede esta cheia
		serv.desconectar()
